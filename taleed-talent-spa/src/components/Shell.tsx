...
import { BookOpen, CalendarDays, Check, CircleHelp, ClipboardList, FileBarChart, FolderOpen, Heart, House, Leaf, Library, LockKeyhole, LogOut, Menu, MessageCircle, Plus, Settings2, ShieldCheck, Users, X } from 'lucide-react';
...
export default function Shell() {
 ...
 <div className="workspace"><header className="topbar"><div className="topbar-start"><Button variant="ghost" className="menu-button" onClick={() => setMenu(true)} aria-label="Open navigation"><Menu /></Button></div><div className="topbar-end"><span className={`save-indicator ${ui.saveStatus}`} role="status"><span className="dot"/>{saveLabel}</span><label className="month-select"><span className="sr-only">Workspace month</span><input type="month" value={month} min="2000-01" max="2100-12" onChange={e => {
            if (e.target.value)
